...
`client/src/types/api/population.d.ts`と`client/src/types/api/prefecture.d.ts`に記載しましょう。

なお、message, resultについては共通のレスポンス項目として`client/src/types/api/api.d.ts`に定義されています。

これに合うように型定義をしましょう。

それができましたら、次は画面用の型定義です。画面上では各都道府県にチェックボックスを入れることができます。インターセクション型を利用して、自分が作った都道府県の型定義に、チェック状態を持つプロパティを追加した、新しい都道府県の型定義を作成しましょう。`client/src/types/display/population.d.ts`に記載できます。
